...
import io.oxiles.integration.broadcast.blockchain.*;
...
public class BlockchainEventBroadcasterConfiguration {
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(name = BROADCASTER_PROPERTY, havingValue = "HCS")
    public BlockchainEventBroadcaster hcsBlockChainEventBroadcaster(HCSSettings settings, Client client, ObjectMapper mapper) {
        final BlockchainEventBroadcaster broadcaster =
                new HCSBlockChainEventBroadcaster(settings, client, mapper);
